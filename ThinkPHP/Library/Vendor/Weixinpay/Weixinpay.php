    ...
    **/
    ...
    public function getParameters(){
        ...
        if (!isset($_GET['code'])) {
            ...
            // 返回的url
            $redirect_uri=U('Weixinpay/pay','','',true);
            ...
            $code=I('get.code');
            // 取出订单号
            $out_trade_no=explode("_",I('get.state'));
			...
			$article = M('sign_list')->where('id='.$out_trade_no[0])->find();
			$shopId = $article['id'];
			$shopTitle = $article['title'];
			//if($out_trade_no[2] == 1){
			//	$price = $article['lcost_s']*100;
			//}else if($out_trade_no[3] == 0){
			//	$price = $article['lcost_x']*100;
			//}

            $order=array(
                'body'=>$shopTitle,// 商品描述（需要根据自己的业务修改）
                'total_fee'=>$out_trade_no[2]*100,// 订单金额  以(分)为单位（需要根据自己的业务修改）
                'out_trade_no'=>$out_trade_no[0].'_'.$out_trade_no[1].'_'.$out_trade_no[4],// 订单号（需要根据自己的业务修改）
                'product_id'=>$shopId,// 商品id（需要根据自己的业务修改）
